...
├── CommonLibrary/          # 公共共享库（Agent-Server通信模型）
├── Monitor-Agent/          # Agent模块（数据采集与上报）
├── Monitor-Server/         # Server模块（数据处理与API）
└── Monitor-Web/            # Web前端模块（Vue 3）
```
...
**父POM依赖管理说明**：
- 父POM (`monitor-parent`) 继承自 `spring-boot-starter-parent:3.5.10`
- 统一管理所有第三方库版本：OSHI、Lombok、Resilience4j、JWT、MyBatis-Plus等
- 子模块无需声明版本号，自动继承父POM配置
- Spring Cloud依赖通过BOM管理，确保版本兼容性
...
# 构建并运行
mvn clean package
java -jar target/Monitor-Agent-0.0.1-SNAPSHOT.jar
```

**模块依赖说明**：
- Agent 依赖 CommonLibrary 模块
- 使用 CommonLibrary 中的 `RegisterRequest`、`BasicReportRequest`、`MetricsReportRequest` 等类与服务端通信
- 父POM自动管理所有依赖版本，无需手动配置
