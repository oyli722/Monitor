...
│       ├── request/         # Agent-Server通信请求DTO
│       │   ├── RegisterRequest.java
│       │   ├── BasicReportRequest.java
│       │   └── MetricsReportRequest.java
│       ├── response/        # Agent-Server通信响应DTO
│       │   ├── BaseResponse.java
│       │   └── RegisterResponse.java
│       └── model/           # 数据传输模型（无MyBatis注解）
│           ├── BasicInfo.java   # 主机硬件信息
│           └── Metrics.java     # 运行时监控指标
│
...
└───────────────────────────┬─────────────────────────────────┘
        ...
        ▼                   ▼                   ▼
┌───────────────┐   ┌───────────────┐   ┌───────────────┐
│ CommonLibrary │◄──│ Monitor-Agent │   │ Monitor-Server│
│   (无依赖)    │   │  依赖Common   │   │ 依赖Common    │
└───────────────┘   └───────────────┘   │  + Agent      │
                                        └───────────────┘
```

**依赖说明**：
- **monitor-parent**: 父POM，继承自 `spring-boot-starter-parent:3.5.10`，统一管理所有依赖版本
- **CommonLibrary**: 最底层模块，无其他模块依赖，包含Agent-Server通信的公共模型
- **Monitor-Agent**: 依赖 CommonLibrary，使用公共请求/响应类与服务端通信
- **Monitor-Server**: 依赖 CommonLibrary 和 Monitor-Agent（需要Agent的部分配置类）
