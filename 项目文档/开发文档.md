...
当前系统配置分散在各个模块的`application.yaml`中，存在以下问题：
- 配置重复（Redis、数据库等共同配置）
- 配置更新需要重启服务
- 多环境配置管理不便
...
| 目标 | 说明 |
|------|------|
| **配置中心** | 抽离Monitor-Server与Monitor-AI的共同配置到Nacos |
| **服务发现** | Agent服务发现，支持多Server负载均衡 |
| **配置管理** | 统一管理AI模型配置、Redis连接等 |
| **动态刷新** | 配置变更支持热更新 |
...
| 配置项 | 当前位置 | 迁移后 |
|--------|----------|--------|
| Redis连接 | Server/AI各自的yaml | Nacos共享配置 |
| AI模型配置 | Monitor-AI/application.yaml | Nacos AI配置 |
| JWT配置 | Server/AI各自的yaml | Nacos安全配置 |
| 数据库配置 | Monitor-Server/application.yaml | Nacos数据源配置 |
...
│  ┌─────────────────────────────────────────────────┐   │
│  │  服务注册与发现                                  │   │
│  │  ├── Monitor-Server (集群节点注册)              │   │
│  │  ├── Monitor-AI (集群节点注册)                  │   │
│  │  └── Agent (订阅Server列表)                     │   │
│  └─────────────────────────────────────────────────┘   │
│  ┌─────────────────────────────────────────────────┐   │
│  │  配置管理                                        │   │
│  │  ├── common-redis.yaml (共享Redis配置)          │   │
│  │  ├── common-jwt.yaml (共享JWT配置)              │   │
│  │  ├── ai-model.yaml (AI模型配置)                 │   │
...
| 1 | 部署Nacos Server | - |
| 2 | 添加Nacos Client依赖 | - |
| 3 | 配置Server和AI模块的Nacos地址 | - |
| 4 | 迁移共同配置到Nacos | 配置分类 |
| 5 | 配置服务发现（Agent订阅Server） | - |
| 6 | 实现配置动态刷新 | `@RefreshScope` |
...
| 问题类型 | 具体表现 | 优化方案 |
|---------|---------|---------|
| 异常处理不统一 | 混用`onErrorReturn`/`onErrorResume` | 统一异常处理器 + 自定义异常 |
| 日志混乱 | 大量DEBUG日志残留 | 清理残留日志，规范化日志级别 |
| 组件暴露 | Controller直接调用`getRedisUtils()` | 移除getter，通过Service封装 |
| 硬编码 | 错误消息、魔法数字 | 常量类 + 国际化支持 |
| JWT未完成 | `getUserIdFromAuth`中TODO | 完善JWT解析逻辑 |
