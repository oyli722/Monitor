...
**架构风格**：客户端-服务端架构，包含四个核心组件。

1. **客户端（Agent）**：部署于被监控主机，负责数据采集与上报
2. **服务端（Server Core）**：系统中枢，负责数据处理、SSH代理与API提供
3. **AI服务（AI Service）**：独立AI服务，负责侧边栏AI助手功能
4. **前端（Web Console）**：提供统一的可视化操作界面
...
│   仪表盘 | 主机监控 | SSH终端 | AI助手                │
│   ├─ request.ts (→ Server:8080)                     │
│   └─ ai-request.ts (→ AI:8081)                     │
└──────────────┬──────────────────┬───────────────────┘
               │                  │
...
│  Agent管理 | 指标存储     │  │   侧边栏AI助手            │
│  SSH代理 | SSH绑定AI      │  │   ChatController         │
│  AgentApiController      │  │   ChatService            │
...
- **服务端发现**：
  - **优先使用ZooKeeper服务发现**：
    - 定时从ZooKeeper拉取服务列表（默认每60秒）
    - 获取所有已注册Server的地址和元数据
    - 本地缓存服务列表，线程安全访问
  - **降级到静态配置**：
    - ZooKeeper不可用时自动降级
    - 使用`agent-config.yaml`中的`server.endpoints`配置
    - 确保服务可用性

- **服务端选择**：
  - 对可用地址发送 HTTP GET 到 `/api/health` 检查连通性
  - 记录响应最快的可用服务端地址
  - 如所有地址均不可用，等待重试（指数退避）
...
- **本地缓存**：
  - 线程安全的`AtomicReference`缓存
  - 提供快速的本地访问
  - 自动更新，无需外部同步

- **降级策略**：
  - ZooKeeper连接失败时自动降级
  - 使用`agent-config.yaml`静态配置
  - 记录降级状态，便于监控

- **负载均衡**：
  - **注册阶段**：健康检查选择响应最快的Server
  - **上报阶段**：随机选择Server实现负载均衡
...
- **ZooKeeper服务注册**：
  - Server启动时自动注册到ZooKeeper
  - 使用临时节点（EPHEMERAL）实现健康检测
  - 节点路径：`/monitor/servers/server-{id}`
  - 元数据存储在`info`子节点（JSON格式）
...
- 两种AI助手场景，满足不同使用需求
- 通用AI对话（侧边栏，独立服务）
- SSH绑定AI助手（主机详情页，与SSH终端集成）

**服务架构**：
- **Monitor-AI (端口8081)**：侧边栏AI助手服务，独立部署
- **Monitor-Server (端口8080)**：SSH绑定AI助手，与SSH终端集成

##### 3.5.1 侧边栏AI助手（Monitor-AI服务）

**服务位置**：Monitor-AI (端口8081)
...
- **主机关联**：可选关联主机到会话（通过Feign调用Server获取Agent信息）
...
| **服务依赖** | 通过Feign调用Server获取Agent信息 | 直接访问本地Agent数据 |
...
### 4.1 Agent注册与上报流程（含ZooKeeper服务发现）

```
┌─────────┐    ┌─────────────┐    ┌─────────┐    ┌─────────────┐
│  Agent  │    │  ZooKeeper   │    │ Server  │    │  ZooKeeper   │
└────┬────┘    └──────┬──────┘    └────┬────┘    └──────┬──────┘
     │                │                  │                │
     │ 1. Server启动注册                 │                │
     │ ──────────────────────────────────────────────────►│
     ...
     │ 5. 检查本地配置                   │                │
     │                │                  │                │
     │ 未注册 → 6. 健康检查发现的servers  │                │
     ├──────────────────────────────────►│                │
     │  7. 选择最快服务器                │                │
     │  ◄───────────────────────────────┤                │
     │                │                  │                │
     │ 8. POST /api/v1/customer/register │                │
     ├──────────────────────────────────►│                │
     │                │          9. 生成agent_id          │
     │                │         10. 保存到数据库           │
     │ 11. {agentId, agentName, token}   │                │
     │◄──────────────────────────────────┤                │
     │                │                  │                │
     │ 12. 保存到本地配置                │                │
     │ 13. 开始定时上报                  │                │
...
**ZooKeeper服务发现特性**：
- Server启动时自动注册，创建临时节点
- Agent每60秒刷新服务列表
- Server宕机时临时节点自动删除
- Agent自动移除失效的Server
- ZooKeeper不可用时降级到静态配置
...
| Redis | - | 缓存 |
| MySQL | 8.0+ | 关系数据库 |
| ZooKeeper | 3.8+ | 服务注册与发现 |
| Curator | 5.7.1 | ZooKeeper客户端 |
| Jackson | - | JSON序列化 |
| Lombok | - | 代码简化 |
| SnakeYAML | - | YAML解析 |
| Resilience4j | - | 容错重试 |
